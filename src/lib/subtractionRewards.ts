// Reward set for the Subtraction Borrow Challenge.
// Reuses existing reward art (mirrors src/lib/additionRewards.ts).

export type SubRewardKind = "coin" | "crown" | "heart" | "diamond" | "energy" | "gem" | "key" | "star";

export const SUB_REWARD_KINDS: SubRewardKind[] = [
  "coin", "crown", "heart", "diamond", "energy", "gem", "key", "star",
];

export const SUB_REWARD_META: Record<SubRewardKind, { src: string; label: string; tint: string }> = {
...
const WEIGHTS: Record<SubRewardKind, number> = {
  coin: 0.55, star: 0.12, crown: 0.08, diamond: 0.07,
  heart: 0.06, energy: 0.05, gem: 0.04, key: 0.03,
};

export const pickSubReward = (): SubRewardKind => {
  const total = SUB_REWARD_KINDS.reduce((s, k) => s + WEIGHTS[k], 0);
  let r = Math.random() * total;
  for (const k of SUB_REWARD_KINDS) {
    r -= WEIGHTS[k];
    if (r <= 0) return k;
  }
  return "coin";
