...
const extractCode = (raw: string): string => {
  ...
  if (!trimmed) return "";
  // If it's a URL or contains /join/<code>, pull last segment
  const match = trimmed.match(/(?:\/join\/)([A-Za-z0-9]+)/);
  if (match) return match[1].toUpperCase();
  // Strip any URL/path noise, take last alnum chunk
...
const JoinClassPage = () => {
  ...
  const refresh = useCallback(async () => {
    ...
    if (!userData.user) {
      navigate(`/auth?redirect=/join${codeParam ? `/${codeParam}` : ""}`);
      return;
    }
  ...
  // Prefill from /join/:code
  useEffect(() => {
    if (codeParam) setJoinCode(codeParam.toUpperCase());
  }, [codeParam]);

  // Realtime: when this user is added as a member → redirect to participant dashboard
  useEffect(() => {
    if (!userId) return;
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-background via-background to-muted/20 text-foreground">
      ...
      <main className="mx-auto w-full max-w-2xl space-y-8 px-6 py-8">
        ...
        <section>
          <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Previous Classes</h2>
          {previous.length === 0 ? (
