...
const difficulties = [
  ...
  { slug: "expert", name: "Expert", desc: "Indices & nested", stars: 4, Icon: Crown,
...
const BidmasHub = () => (
  <main className="relative min-h-screen overflow-hidden bg-background text-foreground animate-fade-in">
    ...
    <header className="relative z-10 flex items-start justify-between p-5 sm:p-8">
      <div>
        <p className="text-xs uppercase tracking-[0.45em] text-amber-400 sm:text-sm">Algebra · Order of Operations</p>
        <h1 className="mt-2 text-3xl font-semibold sm:text-5xl">BIDMAS Workspace</h1>
        <p className="mt-1 text-sm text-muted-foreground">Solve expressions line by line. Equivalence wins.</p>
      </div>
    ...
    <section className="relative z-10 mx-auto grid max-w-4xl gap-4 px-6 pb-10 pt-2 sm:grid-cols-2 lg:grid-cols-4">
      {difficulties.map(({ slug, name, desc, stars, Icon, glow, badge }) => (
        <Link key={slug} to={`/games/bidmas/${slug}`}
