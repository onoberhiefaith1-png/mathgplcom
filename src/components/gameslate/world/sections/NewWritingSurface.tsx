...
function Parchment({ width, height, maps }: Omit<Props, "kind" | "accent" | "colour">) {
  ...
  useExpansion(width, height, (w, h) => {
    if (centre.current) centre.current.scale.set(w / baseWidth, h / baseHeight, 1);
...
function Royal({ width, height, maps }: Omit<Props, "kind" | "accent" | "colour">) {
  ...
  useExpansion(width, height, (w, h) => {
    if (body.current) body.current.scale.set(w / width, h / height, 1);
...
function Crystal({ width, height }: Omit<Props, "kind" | "accent" | "colour" | "maps">) {
  ...
  useExpansion(width, height, (w, h) => {
    ...
    horizontal.current.forEach((node, index) => { if (node) { node.position.set(0, (index ? 1 : -1) * (h / 2 - cut * 0.3), 0.07); node.scale.x = w / width; } });
    vertical.current.forEach((node, index) => { if (node) { node.position.set((index ? 1 : -1) * (w / 2 - cut * 0.3), 0, 0.07); node.scale.y = h / height; } });
  });
  return (
    <group>
...
function Magical({ width, height, maps }: Omit<Props, "kind" | "colour" | "accent">) {
  ...
  useExpansion(width, height, (w, h) => { if (body.current) body.current.scale.set(w / width, h / height, 1); if (left.current) left.current.position.x = -w / 2 - 0.08; if (right.current) right.current.position.x = w / 2 + 0.08; });
...
function Silk({ width, height, maps }: Pick<Props, "width" | "height" | "maps">) {
  ...
  useExpansion(width, height, (w, h) => { if (body.current) body.current.scale.set(w / width, h / height, 1); if (left.current) left.current.position.x = -w / 2 - 0.06; if (right.current) right.current.position.x = w / 2 + 0.06; });
...
export function NewWritingSurface({ kind, width, height, maps, accent, colour }: Props) {
