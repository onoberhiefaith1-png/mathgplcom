...
export interface MulFlyingEvent {
  id: number;
  kind: MulRewardKind;
...
interface Props {
  events: MulFlyingEvent[];
  onArrived?: (id: number, kind: MulRewardKind) => void;
}

interface Flight extends MulFlyingEvent { started: number; }
const DURATION = 850;
...
export const FlyingRewards = ({ events, onArrived }: Props) => {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[60]">
      {flights.map((f) => {
        const meta = MUL_REWARD_META[f.kind];
        return (
          <img
            key={f.id}
            src={meta.src}
            ...
            style={{
              ...
              animation: `mul-fly-${f.id} ${DURATION}ms cubic-bezier(0.45,0,0.55,1) forwards`,
            }}
          />
        );
      })}
