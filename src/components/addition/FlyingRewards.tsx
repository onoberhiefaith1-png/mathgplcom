...
export interface FlyingRewardEvent {
  id: number;
  kind: AddRewardKind;
...
interface Props {
  events: FlyingRewardEvent[];
  onArrived?: (id: number, kind: AddRewardKind) => void;
}

interface Flight extends FlyingRewardEvent {
  started: number;
}
...
export const FlyingRewards = ({ events, onArrived }: Props) => {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[60]">
      {flights.map((f) => {
        const meta = ADD_REWARD_META[f.kind];
        return (
          <img
            key={f.id}
            src={meta.src}
            ...
            style={{
              ...
              animation: `add-fly-${f.id} ${DURATION}ms cubic-bezier(0.45,0,0.55,1) forwards`,
            }}
          />
        );
      })}
