...
const StructureCard = ({ counts, onChange, onConfirm, locked }: Props) => {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-3 space-y-2 shadow-sm">
      <p className="text-[9px] uppercase tracking-[0.28em] text-slate-500">Lesson structure</p>
      ...
      <ul className="divide-y divide-slate-150">
        {STRUCTURE_ROWS.map((kind) => (
          <li key={kind} className="flex items-center justify-between py-1.5">
            <span className="text-[12.5px] font-medium text-slate-800">{rowLabel(kind)}</span>
            <span className="flex items-center gap-1">
              ...
              <input
                ...
                className="w-10 rounded-md border border-slate-300 bg-white px-1 py-0.5 text-center text-[13px] font-semibold text-slate-900 outline-none focus:border-slate-600"
      ...
      {!locked && (
        <Button
          size="sm"
          className="h-7 w-full text-[11.5px] bg-slate-900 text-white hover:bg-slate-800"
          onClick={onConfirm}
        >
          Use this structure
        </Button>
