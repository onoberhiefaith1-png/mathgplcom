...
export interface DivFlyingEvent {
  id: number;
  kind: DivRewardKind;
...
interface Props {
  events: DivFlyingEvent[];
  onArrived?: (id: number, kind: DivRewardKind) => void;
}

interface Flight extends DivFlyingEvent { started: number; }
const DURATION = 850;
...
export const FlyingRewards = ({ events, onArrived }: Props) => {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[60]">
      {flights.map((f) => {
        const meta = DIV_REWARD_META[f.kind];
        return (
          <img
            key={f.id}
            src={meta.src}
            ...
            style={{
              ...
              animation: `div-fly-${f.id} ${DURATION}ms cubic-bezier(0.45,0,0.55,1) forwards`,
            }}
          />
        );
      })}
