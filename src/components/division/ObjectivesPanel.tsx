...
export interface DivObjective {
  kind: DivRewardKind;
  collected: number;
  target: number;
}

export const ObjectivesPanel = ({ objectives }: { objectives: DivObjective[] }) => (
  <div className="rounded-xl border-2 border-primary/30 bg-card/70 p-2.5 backdrop-blur shadow-lg">
    ...
    <ul className="grid grid-cols-2 lg:grid-cols-1 gap-2">
      {objectives.map((o) => {
        ...
        return (
          <li key={o.kind} className="flex items-center gap-2" data-div-objective={o.kind}>
            <img
