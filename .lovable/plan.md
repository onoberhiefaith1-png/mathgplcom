...
- `src/lib/games/types.ts`: add a small helper `loopAt(scenes, t)` style lookup reuse (`checkpointAt` already exists) — no schema or data-shape change; loop ownership is positional in `scenes[]`.
